...
class ModExterna(QMainWindow):
    def __init__(self, cambiar_ventana):
        super().__init__()
        self.cambiar_ventana = cambiar_ventana
        self.controller = ModExternaController()
        self.bloques = []
        self.num_claves = 0
        ...
        titulo = QLabel("Ciencias de la Computación II - Método Módulo (Externa)")
    ...
    def guardar_estructura(self):
        ruta, _ = QFileDialog.getSaveFileName(self, "Guardar estructura", "mod_externa.json", "JSON (*.json)")
        if ruta:
            try:
